...
const onKeydown = (e) => {
  ...
  else if (e.key === 'ArrowUp') { wi -= 1; }
  else if (e.key === 'ArrowDown') { wi += 1; }
  else return;
  e.preventDefault();
  // нормализуем
  if (wi < 0) wi = 0;
  if (wi > props.weeks.length - 1) wi = props.weeks.length - 1;
  if (di < 0) { di = 6; wi = Math.max(0, wi - 1); }
  if (di > 6) { di = 0; wi = Math.min(props.weeks.length - 1, wi + 1); }
...
.day.selected { background:#2b6cb0; color:white; font-weight:700; }
.day.today { box-shadow: inset 0 0 0 1px #2b6cb0; }
.day:hover { transform: translateY(-2px); transition:0.12s; }
.day-num { display:inline-block; min-width:20px; }
</style>
